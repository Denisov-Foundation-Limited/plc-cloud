...
export class DevicesDb {
  constructor({ dataDir, defaultObjects = [] }) {
    ...
    this.allowedIcons = new Set(['apartment', 'house', 'dacha', 'garage', 'garden']);
  }
  ...
  async createObject(name, icon = this.defaultObjectIcon) {
    ...
    const data = await this.readData();
    if (data.objects.some(row => String(row?.name || row) === objectName)) {
  ...
  async deleteObject(name) {
    ...
    if (data.devices.some(row => String(row.object_name || '') === objectName)) {
      throw new Error('object_has_devices');
    }
    data.objects = data.objects.filter(row => String(row?.name || row) !== objectName);
    await this.writeData(data);
  }
  ...
  async setObjectIcon(name, icon) {
    ...
    const data = await this.readData();
    const index = data.objects.findIndex(row => String(row?.name || row) === objectName);
    if (index < 0) {
      throw new Error('object_not_found');
    }
    const normalizedIcon = this.normalizeIcon(icon);
  ...
  async listAllDevices() {
    ...
    return [...data.devices].sort((a, b) => {
      const obj = String(a.object_name || '').localeCompare(String(b.object_name || ''));
      if (obj !== 0) return obj;
  ...
  async createDevice({ device_id, name, api_key, object_name }) {
    const data = await this.readData();
    const key = api_key && String(api_key).trim() ? api_key : generateApiKey();
  ...
  async updateDevice({ currentDeviceId, device_id, name, object_name }) {
    ...
    if (nextDeviceId !== currentDeviceId && data.devices.some(r => r.device_id === nextDeviceId)) {
      throw new Error('device_id_exists');
    }
  ...
  ensureObject(data, name, icon = this.defaultObjectIcon) {
    ...
    const existing = data.objects.find(row => String(row?.name || row) === objectName);
    if (existing) {
